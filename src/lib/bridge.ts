...
export function getBridgeConfig(): BridgeConfig | null {
  ...
  try {
    ...
    if (typeof config.origin === "string" && /^http:\/\/127\.0\.0\.1:\d+$/.test(config.origin) && typeof config.token === "string" && config.token.length > 0) {
...
export async function bridgeFetch(path: string, init: RequestInit = {}): Promise<Response> {
  ...
  return fetch(url, { ...init, headers, targetAddressSpace: config ? "loopback" : undefined } as RequestInit);
}
